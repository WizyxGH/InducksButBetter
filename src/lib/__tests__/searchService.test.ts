...
describe('searchService', () => {
  describe('buildAdvancedSearchQuery', () => {
    it('generates a simple title search query with correct parameters', () => {
      ...
      // 9 trailing langs: one per localised placeholder of the SELECT —
      // series_title, subseries_code, translated_title, story_title (twice:
      // the IN (?, 'en') filter and the ORDER BY preference), story_thumb,
      // full_description, character_list, hero_name.
      expect(result.params).toEqual(['%Duck%', '%Duck%', '%Duck%', '%Duck%', '%Duck%', 24, 0, ...Array(9).fill('fr')]);
    });
